...
<template>
  <dialog
    ...
    open
    aria-modal="true"
    aria-labelledby="technical-dialog-title"
    ...
    <div class="technical-dialog-shell">
      <header class="technical-dialog-header">
        <div>
          ...
          <h3 id="technical-dialog-title">{{ stream?.name || 'Captured stream' }}</h3>
          <p class="technical-dialog-subtitle">Full technical context for {{ stream?.host || 'this captured request' }}.</p>
        </div>
        <button
          ref="closeButton"
          class="icon-button"
